...
describe('idleAlarm — wenn das Auto im Stehen zu viel verliert', () => {
  ...
  it('warnt, wenn der Verlust die Schwelle überschreitet', () => {
    const a = idleAlarm(stats({ socPerDay: 3.4, kwhPerDay: 2.8 }), IDLE_ALARM_KWH_PER_DAY);
  ...
  it('warnt NICHT auf eine bloße Obergrenze', () => {
    ...
    expect(
      idleAlarm(stats({ socPerDay: 4, kwhPerDay: 3.3, obergrenze: true }), IDLE_ALARM_KWH_PER_DAY),
    ).toBeUndefined();
  });
  ...
  it('warnt nicht auf zu kurzer Beobachtung', () => {
    ...
    expect(
      idleAlarm(stats({ socPerDay: 5, kwhPerDay: 4.2, observedDays: 1 }), IDLE_ALARM_KWH_PER_DAY),
    ).toBeUndefined();
  });

  it('schweigt ohne Auswertung', () => {
    expect(idleAlarm(undefined, IDLE_ALARM_KWH_PER_DAY)).toBeUndefined();
  });

  it('lässt sich die Schwelle vorgeben', () => {
    // Wer sein Auto kennt, setzt sie enger — oder schaltet sie mit 0 ab.
    // Gemessen wird in Kilowattstunden: Prozentpunkte hängen an der
    // Batteriegröße, der Ruheverlust selbst nicht.
    expect(idleAlarm(stats({ kwhPerDay: 1.2 }), 1)).toBeDefined();
    expect(idleAlarm(stats({ kwhPerDay: 1.2 }), 0)).toBeUndefined();
  });
